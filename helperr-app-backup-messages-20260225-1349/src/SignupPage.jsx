      ...
      <div style={{paddingTop:70}}>
        <div style={{position:'relative',overflow:'hidden',padding:'60px 20px'}}>
          ...
          <div style={{maxWidth:500,margin:'0 auto',textAlign:'center',position:'relative',zIndex:2,color:'#1F2937'}}>
            <h1 style={{fontSize:48,fontWeight:800,marginBottom:12,fontFamily:'"Outfit",sans-serif',letterSpacing:'-1px'}}>✨ Registrieren</h1>
            <p style={{fontSize:18,fontFamily:'"Outfit",sans-serif',fontWeight:400}}>Erstelle deinen Account</p>
          </div>
        </div>
        ...
        <div style={{maxWidth:500,margin:'-40px auto 80px',padding:'0 20px'}}>
          <form onSubmit={handleSubmit} style={{backgroundColor:'white',borderRadius:20,padding:40,boxShadow:'0 8px 30px rgba(0,0,0,0.1)'}}>
            ...
            <div style={{marginBottom:24}}>
              ...
              <input required type="text" value={name} onChange={(e)=>setName(e.target.value)} style={{width:'100%',padding:'14px 18px',border:'1px solid #E5E7EB',borderRadius:12,fontSize:15,outline:'none',fontFamily:'"Outfit",sans-serif'}}/>
            </div>
            ...
            <div style={{marginBottom:32}}>
              <label style={{display:'block',marginBottom:8,fontWeight:600,fontSize:15,color:'#1F2937',fontFamily:'"Outfit",sans-serif'}}>Passwort * (mind. 6 Zeichen)</label>
              <input required type="password" minLength={6} value={password} onChange={(e)=>setPassword(e.target.value)} style={{width:'100%',padding:'14px 18px',border:'1px solid #E5E7EB',borderRadius:12,fontSize:15,outline:'none',fontFamily:'"Outfit",sans-serif'}}/>
            </div>
            ...
            <p style={{textAlign:'center',fontSize:14,color:'#6B7280',fontFamily:'"Outfit",sans-serif'}}>
              Schon registriert? <button type="button" onClick={()=>window.navigateTo('login')} style={{background:'none',border:'none',color:'#14B8A6',fontWeight:600,cursor:'pointer',textDecoration:'underline'}}>Einloggen</button>
            </p>
          </form>
        </div>
      </div>
