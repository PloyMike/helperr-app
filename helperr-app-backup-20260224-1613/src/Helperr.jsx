...
function Helperr() {
  ...
  return (
    <div style={{minHeight:'100vh',backgroundColor:'#F9FAFB'}}>
      ...
      <div style={{backgroundColor:'white',borderBottom:'1px solid #E5E7EB',padding:'24px 20px'}}>
        <div style={{maxWidth:1200,margin:'0 auto',display:'flex',gap:12,flexWrap:'wrap',alignItems:'center'}}>
          <span style={{fontWeight:700,marginRight:10,fontFamily:'"Outfit",sans-serif',color:'#1F2937',fontSize:16}}>Stadt:</span>
          {cities.map(city=><button key={city} onClick={()=>setSelectedCity(city)} style={{padding:'10px 20px',border:'none',borderRadius:12,backgroundColor:selectedCity===city?'#14B8A6':'#F3F4F6',color:selectedCity===city?'white':'#6B7280',cursor:'pointer',fontWeight:selectedCity===city?600:500,transition:'all 0.2s',fontFamily:'"Outfit",sans-serif',fontSize:14}}>{city==='all'?'Alle Städte':city}</button>)}
        </div>
      </div>
